...
public class SchemaIntegrationTests
{
    ...
    {
        using var db = IntDb.Create();
        var user = new User { Username = "ip_test", FullName = "T", Role = "admin" };
    ...
    {
        using var db = IntDb.Create();
        var user = new User { Username = "idx_test", FullName = "T", Role = "admin" };
        db.Users.Add(user); db.SaveChanges();
...
{
    [Fact]
    public async Task FullRiskLifecycle_ProposedToActionPlanned()
    {
        using var db  = IntDb.Create();
        ...
        // Kullanıcılar
        var owner     = new User { Username = "e2e_owner",     FullName = "Owner",     Role = "risk_owner"   }; db.Users.Add(owner);
        var mgr       = new User { Username = "e2e_mgr",       FullName = "Manager",   Role = "risk_manager" }; db.Users.Add(mgr);
        var committee = new User { Username = "e2e_committee", FullName = "Committee", Role = "committee"    }; db.Users.Add(committee);
        db.SaveChanges();
        ...
        // 1. Risk öner
        var risk = await svc.CreateAsync("E2E Risk", "Senaryo testi", "Finansal", null, null, owner.Id, owner.FullName);
        Assert.Equal("proposed", risk.Status);

        // 2. İncelemeye al
        Assert.True(await svc.UpdateStatusAsync(risk.Id, "under_review", null, mgr));

        // 3. İlk değerlendirme (skor >= 70 → under_review kalır)
        await svc.AddEvaluationAsync(risk.Id, "initial", 10, 6, 40, null, mgr.Id); // skor = 2400
        Assert.Equal("under_review", db.Risks.Find(risk.Id)!.Status);

        // 4. Onaya gönder
        Assert.True(await svc.UpdateStatusAsync(risk.Id, "awaiting_approval", null, mgr));
        ...
        await svc.UpdateMetadataAsync(risk.Id, org.Id, null, "Riski Azaltma", mgr.Id);
        Assert.Equal("strategy_set", db.Risks.Find(risk.Id)!.Status);

        // 7. Kontrol ekle
        await svc.AddControlAsync(risk.Id, "Çok faktörlü kimlik doğrulama", "Önleyici", "Tatmin Edici", "Sürekli", mgr.Id);
        Assert.Equal("controlled", db.Risks.Find(risk.Id)!.Status);

        // 8. Kalan risk değerlendir
        await svc.AddEvaluationAsync(risk.Id, "residual", 1, 1, 7, null, mgr.Id);
        Assert.Equal("residual_evaluated", db.Risks.Find(risk.Id)!.Status);

        // 9. Aksiyon planla
        await svc.AddActionAsync(risk.Id, "Çalışan eğitimi", "IT", DateOnly.FromDateTime(DateTime.Today.AddMonths(3)), mgr.Id);
    ...
    [Fact]
    public async Task FullAuditLifecycle_OpenToClosedViaClosureRequest()
    {
        using var db  = IntDb.Create();
        var svc = new AuditService(db);

        var auditor = new User { Username = "e2e_aud",  FullName = "Auditor", Role = "auditor"       }; db.Users.Add(auditor);
        var mgr     = new User { Username = "e2e_amgr", FullName = "Mgr",     Role = "audit_manager" }; db.Users.Add(mgr);
        db.SaveChanges();

        // Denetim & bulgu oluştur
        var audit   = await svc.CreateAuditAsync("E2E Denetim", "ordinary", "BT", null, "2026", null, null, auditor.Id);
        var finding = await svc.CreateFindingAsync("Parola politikası yok", null, "Bilgi Teknolojileri", "Yüksek", null, auditor.Id, null, audit.Id, DateOnly.FromDateTime(DateTime.Today.AddMonths(1)));
        Assert.Equal("open", finding.Status);

        // Aksiyon planla
        await svc.InternalAddFindingActionAsync(finding.Id, "Parola politikası hazırla", "BT", null, auditor.Id);
        Assert.Equal("action_planned", db.AuditFindings.Find(finding.Id)!.ActionDecision);

        // Kapanış başvurusu
        var req = await svc.InternalSubmitClosureRequestAsync(finding.Id, "Politika yayınlandı", null, auditor.Id);
        Assert.Equal("closure_requested", db.AuditFindings.Find(finding.Id)!.Status);

        // Müdür onaylar
        await svc.InternalReviewClosureRequestAsync(finding.Id, req!.Id, "approved", "Politika dokümanı incelendi", mgr.Id);
